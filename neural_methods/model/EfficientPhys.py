...
#         # SCSA layers for each TSM block
#         self.SCSA_1 = SCSA(dim=nb_filters1, head_num=4, window_size=5,
#                            group_kernel_sizes=[3, 5, 7, 9], qkv_bias=False,
#                            fuse_bn=False, norm_cfg=dict(type='BN'), act_cfg=dict(type='ReLU'),
#                            down_sample_mode='avg_pool', attn_drop_ratio=0., gate_layer='sigmoid')

#         self.SCSA_2 = SCSA(dim=nb_filters1, head_num=4, window_size=5,
#                            group_kernel_sizes=[3, 5, 7, 9], qkv_bias=False,
#                            fuse_bn=False, norm_cfg=dict(type='BN'), act_cfg=dict(type='ReLU'),
#                            down_sample_mode='avg_pool', attn_drop_ratio=0., gate_layer='sigmoid')

#         self.SCSA_3 = SCSA(dim=nb_filters2, head_num=4, window_size=7,
#                            group_kernel_sizes=[3, 5, 7, 9], qkv_bias=False,
#                            fuse_bn=False, norm_cfg=dict(type='BN'), act_cfg=dict(type='ReLU'),
#                            down_sample_mode='avg_pool', attn_drop_ratio=0., gate_layer='sigmoid')

#         self.SCSA_4 = SCSA(dim=nb_filters2, head_num=4, window_size=7,
#                            group_kernel_sizes=[3, 5, 7, 9], qkv_bias=False,
#                            fuse_bn=False, norm_cfg=dict(type='BN'), act_cfg=dict(type='ReLU'),
#                            down_sample_mode='avg_pool', attn_drop_ratio=0., gate_layer='sigmoid')
...
#         # TSM and SCSA block 1
#         network_input = self.TSM_1(inputs)
#         d1 = torch.tanh(self.motion_conv1(network_input))
#         d1 = self.SCSA_1(d1)  # SCSA applied after TSM_1

#         # TSM and SCSA block 2
#         d1 = self.TSM_2(d1)
#         d2 = torch.tanh(self.motion_conv2(d1))
#         d2 = self.SCSA_2(d2)  # SCSA applied after TSM_2
...
#         # TSM and SCSA block 3
#         d4 = self.TSM_3(d4)
#         d5 = torch.tanh(self.motion_conv3(d4))
#         d5 = self.SCSA_3(d5)  # SCSA applied after TSM_3

#         # TSM and SCSA block 4
#         d5 = self.TSM_4(d5)
#         d6 = torch.tanh(self.motion_conv4(d5))
#         d6 = self.SCSA_4(d6)  # SCSA applied after TSM_4

#         # Attention and gated mechanism
#         g2 = torch.sigmoid(self.apperance_att_conv2(d6))
#         g2 = self.attn_mask_2(g2)
#         gated2 = d6 * g2

#         # Final pooling, dropout, and dense layers
#         d7 = self.avg_pooling_3(gated2)
#         d8 = self.dropout_3(d7)
#         d9 = d8.view(d8.size(0), -1)
#         d10 = torch.tanh(self.final_dense_1(d9))
...
import torch.nn as nn
from .SCSA import SCSA
from .WTConv import WTConv2d, DepthwiseSeparableConvWithWTConv2d
...
class EfficientPhys(nn.Module):
    def __init__(self, in_channels=3, nb_filters1=32, nb_filters2=64, kernel_size=3, dropout_rate1=0.25,
        ...
        # 使用 WTConv2d 替换原有卷积层
        self.motion_conv1 = DepthwiseSeparableConvWithWTConv2d(self.in_channels, self.nb_filters1, kernel_size=self.kernel_size)
        self.motion_conv2 = DepthwiseSeparableConvWithWTConv2d(self.nb_filters1, self.nb_filters1, kernel_size=self.kernel_size)
        self.motion_conv3 = DepthwiseSeparableConvWithWTConv2d(self.nb_filters1, self.nb_filters2, kernel_size=self.kernel_size)
        self.motion_conv4 = DepthwiseSeparableConvWithWTConv2d(self.nb_filters2, self.nb_filters2, kernel_size=self.kernel_size)
        ...
        if img_size == 36:
            ...
            self.final_dense_1 = nn.Linear(20736, self.nb_dense, bias=True)
        elif img_size == 96:
            self.final_dense_1 = nn.Linear(30976, self.nb_dense, bias=True)
        else:
            raise Exception('Unsupported image size')
