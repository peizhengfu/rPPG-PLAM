...
#         self.min_valid_loss = None
#         self.best_epoch = 0
#         self.accumulation_steps = config.TRAIN.ACCUMULATION_STEPS  # 梯度累积步数
#         self.scaler = torch.cuda.amp.GradScaler()  # 初始化 GradScaler
...
#                     pred_ppg = self.model(data)
#                     loss = self.criterion(pred_ppg, labels)
#                     loss = loss / self.accumulation_steps  # 梯度累积: 将损失除以累积步数
...
#                 if idx % 100 == 99:  # 每 100 mini-batches 打印一次
#                     print(f'[{epoch}, {idx + 1:5d}] loss: {running_loss / 100:.3f}')
#                     running_loss = 0.0
                
#                 train_loss.append(loss.item() * self.accumulation_steps)
#                 tbar.set_postfix({"loss": loss.item() * self.accumulation_steps, "lr": self.optimizer.param_groups[0]["lr"]})
